...
#    import pdb;pdb.set_trace()
    ...
    # Partial-pixel shift
        ...
        slice0 = slice(int(round(mlarge/4.)),int(round(mlarge/4.*3)))
        slice1 = slice(int(round(nlarge/4.)),int(round(nlarge/4.*3)))
        
        CClarge[slice0,slice1] = fftshift(buf1ft) * conj(fftshift(buf2ft));
